...
re_dic <- data.frame(matrix(nrow = 6, ncol = 3))
colnames(re_dic) <- c("Model", "DIC", "delta_DIC")
...
re_dic[1,1] <- c("Model 1 - intercepts for G, M, PE") #format(m.4.5)
re_dic[2,1] <-  c("Model 2 - quadratic slopes for G, M, PE") #paste0(format(m4.1)[1], trimws(format(m4.1)[2]), trimws(format(m4.1)[3]))
re_dic[3,1] <-  c("Model 3 - linear slopes for G, M, PE") #paste0(format(m4.2)[1], trimws(format(m4.2)[2]))
re_dic[4,1] <- c("Model 4 - intercept for G, linear slopes for M, PE") #format(m4.4)
re_dic[5,1] <-  c("Model 5 - intercept for M, linear slopes for G, PE") #format(m4.3)[1]
re_dic[6,1] <- c("Model 6 - intercept for PE, linear slopes for G, M")#paste0(format(m4.6)[1], trimws(format(m4.6)[2]))
...
re_dic[1,3] <- mod.4.5.DA$DIC - mod.4.5.DA$DIC
re_dic[2,3] <- mod.4.5.DA$DIC - mod.4.1.DA$DIC
re_dic[3,3] <- mod.4.5.DA$DIC - mod.4.2.DA$DIC
re_dic[4,3] <- mod.4.5.DA$DIC - mod.4.4.DA$DIC
re_dic[5,3] <- mod.4.5.DA$DIC - mod.4.3.DA$DIC
re_dic[6,3] <- mod.4.5.DA$DIC - mod.4.6.DA$DIC
```
...
pander(re_dic,
       justify = "lcc",
       split.cell = 80,
       caption = "Table S1 DIC of six intercept models with different combinations of random effects structures for additive genetic variance, maternal variance and permanent environmental variance")
```
...
Table_S2 <- mod.5.2.DA.coeffs

#Create Variable names
Table_S2$Variable[1:4] <- c("Intercept", "Treatment", "Age", "Age^2")

Table_S2$Variable[5:10] <- c("V_g intercept", "V_g  slope", "V_g  curvature", "Cov_g intercept - slope", 
                            "Cov_g intercept - curvature", "Cov_g slope - curvature")

Table_S2$Variable[11:16] <- c("V_m intercept", "V_m  slope", "V_m curvature", "Cov_m intercept - slope", 
...
#Model output coefficients for random intercept model
hot.intercept.DA <- readRDS("~/Dropbox/1 - PhD/4 - ldeli_growthrate/output/rds/hot.intercept.DA")
cold.intercept.DA <- readRDS("~/Dropbox/1 - PhD/4 - ldeli_growthrate/output/rds/cold.intercept.DA")

summary(hot.intercept.DA)
summary(cold.intercept.DA)

#Create table
intercept.tab <- data.frame(matrix(nrow = 5, ncol = 7))
#Names
colnames(intercept.tab) <- c("Variable", "HOT_Estimate", "HOT_Lower", "HOT_Upper", 
                             "COLD_Estimate", "COLD_Lower", "COLD_Upper")
intercept.tab$Variable <- c("Fixed effects Intercept", "Variance G", "Variance Dam", "Variance PE", "h2")

#Fill in table
intercept.tab[1, 2:4] <- round(posterior_summary(hot.intercept.DA$Sol[,"(Intercept)"])[-2],4)
intercept.tab[2, 2:4] <- round(posterior_summary(hot.intercept.DA$VCV[,"liz_id"])[-2],4)
intercept.tab[3, 2:4] <- round(posterior_summary(hot.intercept.DA$VCV[,"dam_id"])[-2],4)
intercept.tab[4, 2:4] <- round(posterior_summary(hot.intercept.DA$VCV[,"id"])[-2],4)

hot_h2 <- hot.intercept.DA$VCV[,"liz_id"]  / (hot.intercept.DA$VCV[,"liz_id"] + hot.intercept.DA$VCV[,"dam_id"] +  hot.intercept.DA$VCV[,"id"] + hot.intercept.DA$VCV[,"units"])
intercept.tab[5, 2:4] <- round(posterior_summary(hot_h2)[-2],4)

intercept.tab[1, 5:7] <- round(posterior_summary(cold.intercept.DA$Sol[,"(Intercept)"])[-2],4)
intercept.tab[2, 5:7] <- round(posterior_summary(cold.intercept.DA$VCV[,"liz_id"])[-2],4)
intercept.tab[3, 5:7] <- round(posterior_summary(cold.intercept.DA$VCV[,"dam_id"])[-2],4)
intercept.tab[4, 5:7] <- round(posterior_summary(cold.intercept.DA$VCV[,"id"])[-2],4)
cold_h2 <- cold.intercept.DA$VCV[,"liz_id"]  / (cold.intercept.DA$VCV[,"liz_id"] + cold.intercept.DA$VCV[,"dam_id"] +  cold.intercept.DA$VCV[,"id"] + cold.intercept.DA$VCV[,"units"])
intercept.tab[5, 5:7] <- round(posterior_summary(cold_h2)[-2],4)

write.csv(intercept.tab, row.names = F, "output/tabs/Table_S4.csv")
```
