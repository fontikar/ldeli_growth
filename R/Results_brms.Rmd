...
Overall, additive genetic variance, permanent environmental variance and heritability of growth appears to be higher in the hot developmental temperature treatment (Fig. XX, Table SXX). However, there were no significant differences among treatment groups (Table XX). Treatment groups did not differ in how the relative contributions of $G$ and $M$ changed with age as their 95% credible intervals overlapped (Fig. 3). Additive genetic variance remained relatively low and constant upon emergence until approximately nine months of age, after which it increased rapidly (Fig. S1). Maternal effects decreasing sharply upon hatching and dropped to the minimum at approximately six months before it increased (Fig. S1). There were some differences among developmental treatments in how residual variance changed with age (Fig. S1). Residual variance in cold incubated lizards showed a reasonably shallow increase  whereas it increased more steeply in hot incubated lizard. After acconting for heterogenous residual variance, we found  no differences in $h^2$ or the proportion of variance explained by maternal effects ($M^2$). Heritability was very low for the first year of growth in L.deliata and only began increasing at one year of age (Fig. XA). As predicted $M^2$ decreased since hatching, however it increased slightly from six months of age (Fig. XB). The $G$ and $M$ matrices for each treatment group are presented in Table S6-S7.
...
hot_cold_Vcomps %>% filter(Treatment == "Cold"  & ! Variable == "h2") %>%
ggplot(aes(y = Estimate, x = "", fill = Variable)) + 
  geom_bar(stat="identity", width=1)  + 
  coord_polar("y", start=0) + 
  scale_fill_manual(values = c("#046C9A", "#D69C4E" ,"#ABDDDE", "#000000")) + 
  theme_void() 

hot_cold_Vcomps %>% filter(Treatment == "Hot"  & ! Variable == "h2") %>%
ggplot(aes(y = Estimate, x = "", fill = Variable)) + 
  geom_bar(stat="identity", width=1)  + 
  coord_polar("y", start=0) + 
  scale_fill_manual(values = c("#046C9A", "#D69C4E" ,"#ABDDDE", "#000000")) + 
...
#Add the WAIC and loo
waic_5.het <- waic(brm_5.het)
waic_5.1.het <- waic(brm_5.1.het)
waic_5.1.5.het <- waic(brm_5.1.5.het)
waic_5.2.het <- waic(brm_5.2.het)

loo_5.het <- loo(brm_5.het)
loo_5.1.het <- loo(brm_5.1.het)
loo_5.1.5.het <- loo(brm_5.1.5.het)
loo_5.2.het <- loo(brm_5.2.het)

#Compare
loo_compare(waic_5.het, waic_5.1.het, waic_5.1.5.het, waic_5.2.het)
loo_compare(loo_5.het, loo_5.1.het, loo_5.1.5.het, loo_5.2.het)
...
The model containing interactions between treatment and the linear and quadratic components of the growth curve was best supported by WAIC values (Table. X - DIC table). The linear growth rate and curvature of the growth trajectory did not differ significantly between the two developmental temperature treatments (Table X - Mod coeffs). Irrespective of treatment, a lizard mass increased by X.XXg for every 1 SD unit increase in age. Developmental temperature did however impact initial mass (Table X - Mod effs, Fig. S). Lizards from the ‘cold’ treatment were on average `r mean(diff)`g (95% CI: `r diff_CI_lower`g – `r diff_CI_upper`g) heavier compared to lizards from the ‘hot’ treatment (Table. 2). Larger initial masses meant that lizards from the ‘cold’ treatment reached their maximum mass slightly earlier (`r posterior_summary(cold_Age_max)[1] %>% round(2)` days, 95% CI: `r posterior_summary(cold_Age_max)[3] %>% round(2)` – `r posterior_summary(cold_Age_max)[4] %>% round(2)`) compared to lizards from ‘hot’ treatment  (`r posterior_summary(hot_Age_max)[1] %>% round(2)` days, 95% CI: `r posterior_summary(hot_Age_max)[3] %>% round(2)` – `r posterior_summary(hot_Age_max)[4] %>% round(2)`). The population G and M matrices and other variance components are presented in Table S5. 


```{r, echo = FALSE, cache = TRUE, warning = FALSE, include = FALSE, cache = TRUE}
#Figure of model predictions showing intercept differences
...
func_growth_predictions <- function(day, predat){
 ...
 # finding dam_id that corresponds to liz_id
  ...
  pred <- brm_5.het_post[, "b_Intercept"] + #Intercept
    brm_5.het_post[, "b_z_days_since_hatch"] * ztran_DsH(day) + #Linear day effect 
    brm_5.het_post[, "b_z_days_since_hatch_I2"] * ((ztran_DsH(day))^2) + #Curve day effect 
    brm_5.het_post[, paste0("r_liz_id[",liz_id, ",Intercept]")] + #Lizard intercept
    brm_5.het_post[, paste0("r_liz_id[",liz_id, ",z_days_since_hatch]")] + #Lizard slope
    brm_5.het_post[, paste0("r_liz_id[",liz_id, ",z_days_since_hatch_I2]")] + #Lizard curve z_days_since_hatch_I2
    brm_5.het_post[, paste0("r_dam_id[",dam_id, ",Intercept]")] + #Dam intercept
    brm_5.het_post[, paste0("r_dam_id[",dam_id, ",z_days_since_hatch]")] + #Dam slope
    brm_5.het_post[, paste0("r_dam_id[",dam_id, ",z_days_since_hatch_I2]")]  #Dam curve
  
  if(treat == 29){
    pred <- pred + 
    brm_5.het_post[, "b_treatment29"] * 1 +
    brm_5.het_post[, "b_treatment29:z_days_since_hatch"] * 1 + 
    brm_5.het_post[, "b_treatment29:z_days_since_hatch_I2"] * 1
  }
...
df <- bind_rows(lapply(days, function(x) func_growth_predictions(x, predat = predat)))
```
...
# rgb(red = 19, green = 146, blue = 119, maxColorValue = 255)
# rgb(red = 238, green = 158, blue = 7, maxColorValue = 255)
```
