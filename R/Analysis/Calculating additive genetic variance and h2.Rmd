...
mod.3.2.DA.pe <- readRDS("./output/rds/mod.3.2.DA.pe")
```
...
x = 1
V_g <- V_liz_g[1] + (x^2)*V_liz_g[2] + (x^4)*V_liz_g[3] + rowSums(2*x*COV_liz_g)
...
# Fonti I stopped here
  COV_dam_g <- data.frame(VCV.3.2.DA) %>%
              select(c(6:8))   

V_dam <- V_liz_g[1] + (x^2)*V_liz_g[2] + (x^4)*V_liz_g[3] + rowSums(2*x*COV_liz_g)
  

h2 <- V_g / (V_g + V_dam + units)
