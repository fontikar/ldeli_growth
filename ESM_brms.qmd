...
########################
  ...
  #Read in models
           models <- lapply(1:16, function(x) readRDS(paste0("./output/rds/brm_", x)))
    names(models) <- paste0("mod_", 1:16)
  ...
  waic_table_comp <- loo_compare(waic_table) 
  lapply(models, function(x) dim(x$data)[1])

  # loo
        loo_table <- lapply(models, function(x) loo(x, moment_match = TRUE))
   loo_table_comp <- loo_compare(loo_table)

  # Lets write all these results to a table
  write_xlsx(list(  r2_all = r2_table,
                  waic_all = waic_table_comp, 
                   loo_all = loo_table_comp), path = "models_results.xlsx")
...
########################
  ...
  #Read in models
           models_cold <- lapply(1:16, function(x) readRDS(paste0("./output/rds/brm_", x, "_cold")))
    names(models_cold) <- paste0("mod_", 1:16, "_cold")
  ...
  waic_table_comp_cold <- loo_compare(waic_table_cold) 
  lapply(models_cold, function(x) dim(x$data)[1])

  # loo
        loo_table_cold <- lapply(models_cold, function(x) loo(x, moment_match = TRUE))
   loo_table_comp_cold <- loo_compare(loo_table_cold)

  # Lets write all these results to a table
  write_xlsx(list(  r2_cold = r2_table_cold,
                  waic_cold = waic_table_comp_cold, 
                   loo_cold = loo_table_comp_cold), path = "models_results_cold.xlsx")
...
########################
  ...
  #Read in models
           models_hot <- lapply(1:16, function(x) readRDS(paste0("./output/rds/brm_", x, "_hot")))
    names(models_hot) <- paste0("mod_", 1:16, "_hot")
  ...
  waic_table_comp_hot <- loo_compare(waic_table_hot) 
  lapply(models_hot, function(x) dim(x$data)[1])

  # loo
        loo_table_hot <- lapply(models_hot, function(x) loo(x, moment_match = TRUE))
   loo_table_comp_hot <- loo_compare(loo_table_hot)

  # Lets write all these results to a table
  write_xlsx(list(  r2_hot = r2_table_hot,
                  waic_hot = waic_table_comp_hot, 
                   loo_hot = loo_table_comp_hot), path = "models_results_hot.xlsx")

# Add loos for comparison. This takes quite a long time so better to load the kfold.Rdata object
rerun = FALSE
  if(rerun){
    loo_mod.1 <- add_criterion(mod.1,  "loo")
    loo_mod.2 <- add_criterion(mod.2,  "loo")
    loo_mod.3 <- add_criterion(mod.3,  "loo")
    loo_mod.4 <- add_criterion(mod.4,  "loo")
    loo_mod.5 <- add_criterion(mod.5,  "loo")
    loo_mod.6 <- add_criterion(mod.6,  "loo")
    loo_mod.7 <- add_criterion(mod.7,  "loo")
    loo_mod.8 <- add_criterion(mod.8,  "loo")
    loo_mod.9 <- add_criterion(mod.9,  "loo")
  }
#Compare
  waic_mods <- loo_compare(w_mod.1, w_mod.2, w_mod.3, w_mod.4, w_mod.5, w_mod.6, w_mod.7, w_mod.8, w_mod.9, w_mod.10, w_mod.11, w_mod.12, w_mod.13, w_mod.14, w_mod.15, w_mod.16)
  write.csv(waic_mods, "./output/tables/fulldata/waic_mods.csv")

  loo_mods <- loo_compare(loo_mod.1, loo_mod.2, loo_mod.3, loo_mod.4, loo_mod.5, loo_mod.6, loo_mod.7, loo_mod.8, loo_mod.9)
  write.csv(loo_mods, "./output/tables/fulldata/loo_mods.csv")

########################
# Seperated data - cold
########################
  mod.1_cold <- readRDS("./output/rds/brm_1_cold")
  mod.2_cold <- readRDS("./output/rds/brm_2_cold")
  mod.3_cold <- readRDS("./output/rds/brm_3_cold")
  mod.4_cold <- readRDS("./output/rds/brm_4_cold")
  mod.5_cold <- readRDS("./output/rds/brm_5_cold")
  mod.6_cold <- readRDS("./output/rds/brm_6_cold")
  mod.7_cold <- readRDS("./output/rds/brm_7_cold")
  mod.8_cold <- readRDS("./output/rds/brm_8_cold") 
  mod.9_cold <- readRDS("./output/rds/brm_9_cold")

#Add waics
    w_mod.1_cold <- waic(mod.1_cold)
    w_mod.2_cold <- waic(mod.2_cold)
    w_mod.3_cold <- waic(mod.3_cold)
    w_mod.4_cold <- waic(mod.4_cold)
    w_mod.5_cold <- waic(mod.5_cold)
    w_mod.6_cold <- waic(mod.6_cold)
    w_mod.7_cold <- waic(mod.7_cold)
    w_mod.8_cold <- waic(mod.8_cold)
    w_mod.9_cold <- waic(mod.9_cold)

# Add loos for comparison

# Add loos for comparison. This takes quite a long time so better to load the kfold.Rdata object
if(rerun){
    loo_mod.1_cold <- add_criterion(mod.1_cold, "kfold",  k = 10)
    loo_mod.2_cold <- add_criterion(mod.2_cold, "kfold",  k = 10)
    loo_mod.3_cold <- add_criterion(mod.3_cold, "kfold",  k = 10)
    loo_mod.4_cold <- add_criterion(mod.4_cold, "kfold",  k = 10)
    loo_mod.5_cold <- add_criterion(mod.5_cold, "kfold",  k = 10)
    loo_mod.6_cold <- add_criterion(mod.6_cold, "kfold",  k = 10)
    loo_mod.7_cold <- add_criterion(mod.7_cold, "kfold",  k = 10)
    loo_mod.8_cold <- add_criterion(mod.8_cold, "kfold",  k = 10)
    loo_mod.9_cold <- add_criterion(mod.9_cold, "kfold",  k = 10)
}

#Compare
  waic_mods_cold <- loo_compare(w_mod.1_cold, w_mod.2_cold, w_mod.3_cold, w_mod.4_cold, w_mod.5_cold, w_mod.6_cold, w_mod.7_cold, w_mod.8_cold, w_mod.9_cold)
  write.csv(waic_mods_cold, "./output/tables/cold/waic_mods_cold.csv")

  loo_mods_cold <- loo_compare(loo_mod.1_cold, loo_mod.2_cold, loo_mod.3_cold, loo_mod.4_cold, loo_mod.5_cold, loo_mod.6_cold, loo_mod.7_cold, loo_mod.8_cold, loo_mod.9_cold)
  write.csv(loo_mods_cold, "./output/tables/cold/loo_mods_cold.csv")

########################
# Seperated data - hot
########################
  mod.1_hot <- readRDS("./output/rds/brm_1_hot")
  mod.2_hot <- readRDS("./output/rds/brm_2_hot")
  mod.3_hot <- readRDS("./output/rds/brm_3_hot")
  mod.4_hot <- readRDS("./output/rds/brm_4_hot")
  mod.5_hot <- readRDS("./output/rds/brm_5_hot")
  mod.6_hot <- readRDS("./output/rds/brm_6_hot")
  mod.7_hot <- readRDS("./output/rds/brm_7_hot")
  mod.8_hot <- readRDS("./output/rds/brm_8_hot") 
  mod.9_hot <- readRDS("./output/rds/brm_9_hot")

#Add waics
    w_mod.1_hot <- waic(mod.1_hot)
    w_mod.2_hot <- waic(mod.2_hot)
    w_mod.3_hot <- waic(mod.3_hot)
    w_mod.4_hot <- waic(mod.4_hot)
    w_mod.5_hot <- waic(mod.5_hot)
    w_mod.6_hot <- waic(mod.6_hot)
    w_mod.7_hot <- waic(mod.7_hot)
    w_mod.8_hot <- waic(mod.8_hot)
    w_mod.9_hot <- waic(mod.9_hot)

# Add loos for comparison

# Add loos for comparison. This takes quite a long time so better to load the kfold.Rdata object
if(rerun){
    loo_mod.1_hot <- add_criterion(mod.1_hot, "kfold",  k = 10)
    loo_mod.2_hot <- add_criterion(mod.2_hot, "kfold",  k = 10)
    loo_mod.3_hot <- add_criterion(mod.3_hot, "kfold",  k = 10)
    loo_mod.4_hot <- add_criterion(mod.4_hot, "kfold",  k = 10)
    loo_mod.5_hot <- add_criterion(mod.5_hot, "kfold",  k = 10)
    loo_mod.6_hot <- add_criterion(mod.6_hot, "kfold",  k = 10)
    loo_mod.7_hot <- add_criterion(mod.7_hot, "kfold",  k = 10)
    loo_mod.8_hot <- add_criterion(mod.8_hot, "kfold",  k = 10)
    loo_mod.9_hot <- add_criterion(mod.9_hot, "kfold",  k = 10)
}

# Save all the R objects in working environment to save time
  save.image("output/kfold/kfold.RData")
...
random_waic[1,1] <- c("Model 7 - intercept for PE, quadratic slopes for G, M") 
random_waic[2,1] <- c("Model 3 - quadratic slopes for G, M, PE")
random_waic[3,1] <- c("Model 6 - intercept for PE, linear slopes for G, M")
random_waic[4,1] <- c("Model 4 - intercept for G, linear slopes for M, PE") 
random_waic[5,1] <- c("Model 2 - linear slopes for G, M, PE")
random_waic[6,1] <- c("Model 5 - intercept for M, linear slopes for G, PE")
random_waic[7,1] <- c("Model 1 - intercepts for G, M, PE")

#wAIC values
random_waic[1,2] <- w_mod.7$estimates[3,1]
random_waic[2,2] <- w_mod.3$estimates[3,1]
random_waic[3,2] <- w_mod.6$estimates[3,1]
random_waic[4,2] <- w_mod.4$estimates[3,1]
random_waic[5,2] <- w_mod.2$estimates[3,1]
...
#Read in model output
brm_5.het <- readRDS("./output/rds/brm_5.het")
