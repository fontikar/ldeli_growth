...
# Lets have a look at model fit. 
  # Histogram of residuals - assumed normal - pretty good to me
   resid <-  mod_12$data$lnMass - predict(mod_12, summary = TRUE)[,"Estimate"]
  resid2 <-  brm_12_het$data$lnMass - predict(brm_12_het, summary = TRUE)[,"Estimate"]
  
  # Look at the residuals - should be normally distributed
    par(mfrow = c(2,2))
    hist(resid, main = "Homogenous residual variance", xlab = "Residuals")
    hist(resid2, main = "Heterogeneous residual variance", xlab = "Residuals")
  ...
  # We already know roughly from R2 that model does good job predciting observed response, but lets have a look. Little bit of over/underpredciting but nothing serious
    ...
    plot(mod_12$data$lnMass ~ predict(mod_12, summary = TRUE)[,"Estimate"], ylab = "Observed lnMass", xlab = "Predicted lnMass", main = "Homogenous residual variance")
    abline(0,1, col = "red")
    plot(brm_12_het$data$lnMass ~ predict(brm_12_het, summary = TRUE)[,"Estimate"], ylab = "Observed lnMass", xlab = "Predicted lnMass", main = "Heterogeneous residual variance")
    abline(0,1, col = "red")
...
cold_brm_4.5 <- readRDS("./output/rds/brm_10_cold")
hot_brm_4.5 <- readRDS("./output/rds/brm_10_hot")
...
Hot_Pheno_intercept <- hot_brm_4.5_post[,"sd_F1_Genotype__Intercept"] +
  hot_brm_4.5_post[,"sd_dam_id__Intercept"] + 
  hot_brm_4.5_post[,"sigma"]

cold_Pheno_intercept <- cold_brm_4.5_post[,"sd_F1_Genotype__Intercept"] +
...
hot_cold_Vcomps[4,3:5] <- ((hot_brm_4.5_post[,"sd_F1_Genotype__Intercept"]^2 / Hot_Pheno_intercept_V) %>% posterior_summary())[-2] %>% round(3)
...
cold_brm_5.5 <- readRDS("./output/rds/brm_12_cold_het") 
 hot_brm_5.5 <- readRDS("./output/rds/brm_12_hot_het")

#Define the days to calculate estimates
z_days <- ztran_DsH(seq(0, 500, 30))

#Get estimates across age for hot treatment
brms_hot_Vg <- lapply(z_days, function(x) brms_Vcomp(model = hot_brm_5.5, x = x, group_var = "F1_Genotype")) %>% bind_rows()
...
brms_cold_Vg <- lapply(z_days, function(x) brms_Vcomp(model = cold_brm_5.5, x = x, group_var = "F1_Genotype")) %>% bind_rows()
...
#Calculate VAR from SD
    hot_G_sd_post <- posterior_samples(hot_brm_5.5, "^sd_F1_Genotype")
   hot_G_var_post <- posterior_samples(hot_brm_5.5, "^sd_F1_Genotype")^2
hot_G_var_summary <- posterior_summary(hot_G_var_post)[,-2] %>% round(3)

#Calculate COV from COR
   hot_G_cor_post <- posterior_samples(hot_brm_5.5, "^cor_F1_Genotype")
...
#Residuals
exp(posterior_samples(hot_brm_5.5, "b_sigma_I"))^2 %>% posterior_summary() %>% signif(digits = 3)
exp(posterior_samples(hot_brm_5.5, "b_sigma_z"))^2 %>% posterior_summary() %>% signif(digits = 3)
...
#Calculate VAR from SD
    cold_G_sd_post <- posterior_samples(cold_brm_5.5, "^sd_F1_Genotype")
   cold_G_var_post <- posterior_samples(cold_brm_5.5, "^sd_F1_Genotype")^2
cold_G_var_summary <- posterior_summary(cold_G_var_post)[,-2] %>% round(3)

#Calculate COV from COR
   cold_G_cor_post <- posterior_samples(cold_brm_5.5, "^cor_F1_Genotype")
...
#Residuals
exp(posterior_samples(cold_brm_5.5, "b_sigma_I"))^2 %>% posterior_summary() %>% signif(digits = 3)
exp(posterior_samples(cold_brm_5.5, "b_sigma_z"))^2 %>% posterior_summary() %>% signif(digits = 3)
```
...
#Read in model output
hot_brm_5.4 <- readRDS("./output/rds/brm_12_hot_het_fixed1") 
cold_brm_5.4 <- readRDS("./output/rds/brm_12_cold_het_fixed1") 

#Obtain CV estimates over age for hot lizards
hot_CV_G <- lapply(seq(0, 500, 30), function(x) get_CV_brms(model = hot_brm_5.4, x = x, group_var = "F1_Genotype")) %>% bind_rows()
...
cold_CV_G <- lapply(seq(0, 500, 30), function(x) get_CV_brms(model = cold_brm_5.4, x = x, group_var = "F1_Genotype")) %>% bind_rows()
