...
#Read in model output
brm_5.het <- readRDS("./output/rds/brm_12_het_fixed1") # Update the model brm_5.het
...
#Model coefficients of the other models
  brm_5.1.het <- readRDS("./output/rds/brm_12_het_fixed2") #brm_5.1.het
brm_5.1.5.het <- readRDS("./output/rds/brm_12_het_fixed3") #brm_5.1.5.het
  brm_5.2.het <- readRDS("./output/rds/brm_12_het_fixed4") #brm_5.2.het
...
```{r}
#| label: tbl-Table_7
#| tbl-cap: Coefficient estimates from model with interaction between treatment and age (linear growth rate) only. Bolded estimates are significantly different from zero. * indicates that value is above zero prior to rounding. $n_{obs}$ = 3002"
...
```{r}
#| label: tbl-Table_8
#| tbl-cap: Coefficient estimates from model with interaction between treatment and quadratic age paramater only. Bolded estimates are significantly different from zero. * indicates that value is above zero prior to rounding. $n_{obs}$ = 3002"
#Outputting it nicely for ESM
flextable(Table_8)  %>%  align(align = "center", part = "all") %>% autofit()
```
...
```{r}
#| label: tbl-Table_9
#| tbl-cap: Coefficient estimates from model with main effects of treatment and age only. Bolded estimates are significantly different from zero. * indicates that value is above zero prior to rounding. $n_{obs}$ = 3002"

#Outputting it nicely for ESM
flextable(Table_9)  %>%  align(align = "center", part = "all") %>% autofit()
```
...
Table_noID <- posterior_summary(mod.5.noid)[1:8,-2] %>% round(2)  %>% data.frame()  %>% mutate(Parameter = rownames(.)) %>% select(Parameter, Estimate:Q97.5) %>% tibble::remove_rownames()
```
...
```{r}
#| label: tbl-Table_10
#| tbl-cap: Coefficient estimates from model with main effects of treatment and age (linear and quadratic) only accounting for maternal effects. Bolded estimates are significantly different from zero. * indicates that value is above zero prior to rounding. $n_{obs}$ = 2926"

#Outputting it nicely for ESM
flextable(Table_noID)  %>%  align(align = "center", part = "all") %>% autofit()
```
#### CHECK ALL BELOW ####
