...
```{r, mod_sel_code, model_select, eval = FALSE, echo = FALSE, warning = FALSE, include = FALSE}
########################
### Load data 
...
# Load in the tables
  tableDA_waic <- readxl::read_xlsx("./output/tables/fulldata/models_results.xlsx", sheet = "waic_all") %>% select(-...5)
   tableDA_loo <- readxl::read_xlsx("./output/tables/fulldata/models_results.xlsx", sheet = "loo_all") %>% select(-...5)
...
```{r, brms_IC_het_code, eval = FALSE, echo = FALSE, warning = FALSE, include = FALSE}
...
#| tbl-cap: Comparisons of expected log predictive density values for LOO and WAIC values to test the importance of  heterogenous and homogenous residual variance. Note that difference in values are calculated by substracting values from the model with lowest LOO and WAIC values i.e. the heterogenous variance model
  table_het <- readxl::read_xlsx("./output/tables/models_results_het.xlsx", sheet = "waic_het_table")
 table_het2 <- readxl::read_xlsx("./output/tables/models_results_het.xlsx", sheet = "loo_het_table")
...
#Extract VCV
brm_12_cold_post <- posterior_samples(brm_12_cold_het) 
 brm_12_hot_post <- posterior_samples(brm_12_hot_het) 

#Calculate total phenotypic variance for each treatment
Hot_Pheno_intercept_V <- brm_12_hot_post[,"sd_F1_Genotype__Intercept"]^2 +
  brm_12_hot_post[,"sd_dam_id__Intercept"]^2 + 
  exp(brm_12_hot_post[,"b_sigma_Intercept"])^2

cold_Pheno_intercept_V <- brm_12_cold_post[,"sd_F1_Genotype__Intercept"]^2 +
  brm_12_cold_post[,"sd_dam_id__Intercept"]^2 + 
  exp(brm_12_cold_post[,"b_sigma_Intercept"])^2
...
# Add in hot to table
hot_cold_Vcomps[1,3:5] <- ((brm_12_hot_post[,"sd_F1_Genotype__Intercept"])^2 %>% posterior_summary())[-2] %>% round(3)
hot_cold_Vcomps[2,3:5] <- ((brm_12_hot_post[,"sd_dam_id__Intercept"])^2 %>% posterior_summary())[-2]%>% round(3)
hot_cold_Vcomps[3,3:5] <- (exp((brm_12_hot_post[,"b_sigma_Intercept"]))^2 %>% posterior_summary())[-2]%>% round(3)
hot_cold_Vcomps[4,3:5] <- ((brm_12_hot_post[,"sd_F1_Genotype__Intercept"]^2 / Hot_Pheno_intercept_V) %>% posterior_summary())[-2] %>% round(3)

# Add in cold
hot_cold_Vcomps[5,3:5] <- ((brm_12_cold_post[,"sd_F1_Genotype__Intercept"])^2 %>% posterior_summary())[-2]%>% round(3)
hot_cold_Vcomps[6,3:5] <- ((brm_12_cold_post[,"sd_dam_id__Intercept"])^2 %>% posterior_summary())[-2]%>% round(3)
hot_cold_Vcomps[7,3:5] <- (exp(brm_12_cold_post[,"b_sigma_Intercept"])^2 %>% posterior_summary())[-2]%>% round(3)
hot_cold_Vcomps[8,3:5] <- ((brm_12_cold_post[,"sd_F1_Genotype__Intercept"]^2 / cold_Pheno_intercept_V) %>% posterior_summary())[-2]%>% round(3)
```

```{r, vcomp_tbl, echo = FALSE}
#| label: tbl-Vcomps
#| tbl-cap: Treatment comparisons of additive genetic, maternal and residual variance and heritability (h2) at an average age. The lower and upper bounds of the 95% credible intervals are provided in brackets.
...
```{r, hot_G, echo = FALSE}
#| label: tbl-hot_G
#| tbl-cap: Variance components of additive genetic variance (V_additive_g) for hot incubated lizards. The lower and upper bounds of the 95% credible intervals are provided in brackets.
...
flextable(Table_4_G) %>% align(align = "center", part = "all") %>% autofit()
```

```{r, hot_M, echo = FALSE}
#| label: tbl-hot_M
#| tbl-cap: Variance components of maternal variance (V_maternal) for hot incubated lizards. The lower and upper bounds of the 95% credible intervals are provided in brackets.
...
flextable(Table_4_M) %>% align(align = "center", part = "all") %>% autofit()
```

```{r, hot_Vr, echo = FALSE}
#| label: tbl-hot_Vr
#| tbl-cap: Variance components of residual variance (V_residual) for hot incubated lizards. The lower and upper bounds of the 95% credible intervals are provided in brackets.
...
flextable(Table_4_E) %>% align(align = "center", part = "all") %>% autofit()
```

```{r, cold_G, echo = FALSE}
#| label: tbl-cold_G
#| tbl-cap: Variance components of additive genetic variance (V_additive_g) for cold incubated lizards. The lower and upper bounds of the 95% credible intervals are provided in brackets.
...
flextable(Table_5_G) %>% align(align = "center", part = "all") %>% autofit()
```

```{r, cold_M, echo = FALSE}
#| label: tbl-cold_M
#| tbl-cap: Variance components of maternal variance (V_maternal) for cold incubated lizards. The lower and upper bounds of the 95% credible intervals are provided in brackets.
...
flextable(Table_5_M) %>% align(align = "center", part = "all") %>% autofit()
```

```{r, cold_Vr, echo = FALSE}
#| label: tbl-cold_Vr
#| tbl-cap: Residual variance (V_residual) for cold incubated lizards. The lower and upper bounds of the 95% credible intervals are provided in brackets.
  
  #Residuals
  cold_e_int <- exp(posterior_samples(brm_12_cold_het, "b_sigma_I"))^2 %>% posterior_summary() %>% signif(digits = 3)
  cold_e_slope <- posterior_samples(brm_12_cold_het, "b_sigma_z")^2 %>% posterior_summary() %>% signif(digits = 3)

      Table_5_E <- data.frame(matrix(nrow = 1, ncol = 3))
  Table_5_E[1,2] <- paste0(cold_e_int[,"Estimate"], " (", cold_e_int[,"Q2.5"], " - ", cold_e_int[,"Q97.5"], ")")
  Table_5_E[1,3] <- paste0(cold_e_slope[,"Estimate"], " (", cold_e_slope[,"Q2.5"], " - ", cold_e_slope[,"Q97.5"], ")")
  write.csv(Table_5_E, "output/tables/Table_4_E_brms_hot.csv")

  flextable(Table_5_E) %>% align(align = "center", part = "all") %>% autofit()
```
...
```{r, CV_code, echo = FALSE, include= FALSE}
...
```{r, CV_plot, echo = FALSE, warning = FALSE, include = FALSE, message = FALSE}
#Plotting CV comparisons of variance components over age

group_var_labs_3 <- c(F1_Genotype = "G",
                      dam_id = "M")

cv <- ggplot(data = brms_CV_compare %>% filter(!group_id == "sigma"), aes(x = day, y = Estimate, colour = factor(treatment))) +
  geom_point(size = 2) + 
...
cv
ggsave(cv, filename = "output/figures/figSCV.pdf", width = 14.679012, height = 5.703704, units = "in")

```

```{r, cv_ratio_code, echo = FALSE, include = FALSE}
# age
age <- seq(0, 450, 30)
...
cv2 <- ggplot(data = CV_ratios, aes(x = day, y = Estimate, colour = factor(treatment))) +
  geom_point(size = 2) + 
  #geom_errorbar(aes(min = Lower, max = Upper), width = 0, alpha = 0.5) + 
  geom_ribbon(aes(ymin = Lower, ymax = Upper), alpha = 0.05) +
  #geom_smooth(aes(ymin = Lower, ymax = Upper), stat = "identity", alpha = 0.1) +
...
cv2 
ggsave(cv2, filename = "output/figures/figSCV2.pdf", width = 14.679012, height = 5.703704, units = "in")
```

## Do growth trajectories differ among developmental temperatures?

To test for treatment differences in growth trajectories, we fitted four models with different combinations of treatment interactions with the linear and quadratic age parameters and compared their LOO/WAIC values (Table 1 og main manuscript). Given that the full model was in the set of top-supported models, we present its G and M matrix is presented in Table S6 below. 

```{r, fixed1_code, echo = FALSE, include = FALSE}
#Compiling fixed effects coefficients for overall model testing for differences in growth curves
#Read in model output
brm_12_het_fixed1 <- readRDS("./output/rds/brm_12_het_fixed1") # Update the model brm_5.het

#Extract entire posterior
brm_12_het_fixed1_post <- posterior_samples(brm_12_het_fixed1)
#Names of each column
colnames(brm_12_het_fixed1_post)[1:21]

#Combining the coefficients from the final model
#Getting the fixed effects
brm_12.fixed <- brm_12_het_fixed1_post[,c(1,3,4,5,6,7)] %>% posterior_summary() %>% round(3) 
#Excluding est. error
brm_12.fixed <- brm_12.fixed[,c(1,3,4)]
```

```{r, fixed1_tbl, echo = FALSE}
#| label: tbl-fixed1_tbl
#| tbl-cap: Table
#Making a table
Table_5 <- brm_12.fixed %>% as.data.frame()
...
flextable(Table_5)   %>%  align(align = "center", part = "all") %>% autofit()
```

```{r, G_fixed1, echo = FALSE, include = FALSE}
#Compiling G matrix for best supported model (5_het)
colnames(brm_12_het_fixed1_post)[1:21]

#Calculate VAR from SD
G_sd_post <- posterior_samples(brm_12_het_fixed1, "^sd_F1_Genotype")
G_var_post <- posterior_samples(brm_12_het_fixed1, "^sd_F1_Genotype")^2
G_var_summary <- posterior_summary(G_var_post)[,-2] %>% round(3)

#Calculate COV from COR
G_cor_post <- posterior_samples(brm_12_het_fixed1, "^cor_F1_Genotype")
G_cor_post
...
```{r, M_fixed1, echo = FALSE, include = FALSE}
#Compiling M matrix for best supported model (5_het)

#Calculate VAR from SD
M_sd_post <- posterior_samples(brm_12_het_fixed1, "^sd_dam_id")
M_var_post <- posterior_samples(brm_12_het_fixed1, "^sd_dam_id")^2
M_var_summary <- posterior_summary(M_var_post)[,-2] %>% round(3)

#Calculate COV from COR
M_cor_post <- posterior_samples(brm_12_het_fixed1, "^cor_dam_id")
...
#Residuals
posterior_samples(brm_12_het_fixed1, "b_sigma_I")^2 %>% posterior_summary() %>% signif(digits = 3)
posterior_summary(brm_12_het_fixed1, "b_sigma_z") %>% signif(digits = 3)

```

## Egg mortality analysis

Egg mortality did not differ significantly between the two treatments (see, @egg_mort_tbl).

```{r, egg_mort_tbl, echo = FALSE}
#| label: tbl-egg_mort_tbl
#| tbl-cap: Analysis of egg mortality between treatmemnts. We fit a Bournoulli generalised linear mixed model with a logit link function to test whether egg mortality differed between treatments. We controlled for clutch ID as a random effect in our analysis. 

# Load model
mort_model <- readRDS("./output/rds/mort_model")

# Create table
Table_7 <- posterior_summary(mort_model)[1:3,]  %>% as.data.frame()  %>% tibble() %>% round(3)

#Names
Table_7$Parameter <-  c("Intercept", "Treatment", "Clutch ID (SD)")  

Table_7 <- Table_7  %>% select(Parameter, Estimate, Q2.5, Q97.5)

flextable(Table_7)  %>%  align(align = "center", part = "all") %>% autofit()

```

## Pedigree statistics across treatments

```{r, ped_stats, echo = FALSE, include = FALSE}
# pedigree_stats
