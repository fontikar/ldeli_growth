...
```{r, model_select, echo = FALSE, warning = FALSE, include = FALSE}
########################
### Load data 
...
########################
  ...
  test <- function(x) {
    print(x)
    loo(x, moment_match = TRUE)
  }
  loo_table_cold <- lapply(models_cold, function(x) test(x))
        loo_table_cold <- lapply(models_cold, function(x) brms::loo(x, moment_match = TRUE))
...
# Could also check with DharMa but less sure the function is correct
  #check_brms(mod_12)
  #check_brms(brm_12_het)

#Compare
  ics <- loo(mod_12, brm_12_het, c("loo", "waic"), moment_match = TRUE)
loo_het_table <- data.frame(loo_compare(ics))
