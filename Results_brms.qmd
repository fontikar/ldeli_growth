...
pacman::p_load(dplyr, magrittr, ggplot2, patchwork, lubridate, stringr, tidyr, MCMCglmm, bookdown, pander, brms, janitor, purrr, tidybayes, stringr, latex2exp, tidybayes, bayestestR, kinship2, xlsx, patchwork, ggrepel, magick)
...
#Define the days
z_days <- ztran_DsH(seq(0, 450, 30))

#Compiling heritability and maternal effects estimates across days
#Hot developmental temperature model
brms_hot_h2 <- lapply(z_days, function(x) brms_h2(model = hot_brm_5.5, z_age = x, G = "F1_Genotype", M = "dam_id")) %>% bind_rows()
brms_hot_m2 <- lapply(z_days, function(x) brms_m2(model = hot_brm_5.5, z_age = x, G = "F1_Genotype", M = "dam_id")) %>% bind_rows()
...
brms_cold_h2 <- lapply(z_days, function(x) brms_h2(model = cold_brm_5.5, z_age = x, G = "F1_Genotype", M = "dam_id")) %>% bind_rows()
brms_cold_m2 <- lapply(z_days, function(x) brms_m2(model = cold_brm_5.5, z_age = x, G = "F1_Genotype", M = "dam_id")) %>% bind_rows()
...
#Table 2
tab2 <- data.frame(read_xlsx("./output/tables/brms_fixed.xlsx") %>% select(-id))

flextable::flextable(tab2)  %>% flextable::width(j = 1, width = 5) 
```
...
While the model containing an full interaction between treatment and linear and quadratic age was best supported, the improvement was marginal and our main effects model explained substantial amount of variation in mass overall ($R^2$ = `r r2_fixed_main[1]`, 95% CI: `r r2_fixed_main[3]` to `r r2_fixed_main[4]`; Table 1). Developmental temperature did impacted initial mass (Table 1, Fig. 3). Lizards from the ???cold??? treatment were, on average, `r diff[,"Estimate"]`g (95% CI: `r diff[,"Q2.5"]`g ??? `r diff[,"Q97.5"]`g) heavier compared to lizards from the ???hot??? treatment (Table. 2). However, the linear growth rate (Age) and nonlinear growth trajectory ($Age^2$) did not differ significantly between the two developmental temperature treatments (Table S5, Table S7-S9). Irrespective of treatment, lizard mass increased by `r growth_rate[1]`g for every 1 SD unit increase in age (95% CI: `r growth_rate[3]` to `r growth_rate[4]`).  
