...
# Check that the same pattern is observed in the combined dataset
 ...
 brms_h2 <- lapply(days, function(x) brms_h2(model = brm_12_het, age = x, G = "F1_Genotype", M = "dam_id", data = data_DA)) %>% bind_rows()
brms_m2 <- lapply(days, function(x) brms_m2(model = brm_12_het, age = x, G = "F1_Genotype", M = "dam_id", data = data_DA)) %>% bind_rows()

#Putting it together in one data frame
brms_h2m2 <- bind_rows(brms_h2, 
                        brms_m2)

#Assigning treatment
brms_h2m2$treatment <- "All"
```
