...
ggsave(filename = "./output/figures/fig1.pdf", width = 13.419753, height = 5.740741)
...
ggplot(data = brms_all, aes(x = day, y = Estimate)) +
  ...
  theme(legend.position = "none",
        strip.text = element_text(size = 18, face = "bold"),
        strip.background = element_blank())
ggsave(filename = "./output/figures/fig1_all.pdf", width = 13.419753, height = 5.740741)
```
...
ranges <- brms_h2m2 %>% group_by(group_id) %>% summarise(min = min(Estimate), max = max(Estimate)) %>% as.data.frame()
#--------------------------------
...
pf
ggsave(pf, filename = "./output/figures/fig2.pdf", width = 10, height = 11)
...
pf2
ggsave(pf2, filename = "./output/figures/fig2_fullData.pdf", width = 10, height = 11)
```
...
p3 <- df %>% filter(liz_id %in% c(hot_plot_ids, cold_plot_ids)) %>%
  ggplot(
  ...
  my_theme
p3
ggsave(p3, filename = "./output/figures/fig3.pdf", width = 10, height = 10)
