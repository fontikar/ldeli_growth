...
pacman::p_load(dplyr, magrittr, ggplot2, patchwork, lubridate, stringr, tidyr, MCMCglmm, bookdown, pander, brms, janitor, purrr, tidybayes, stringr, latex2exp, tidybayes, bayestestR, kinship2, xlsx, patchwork, ggrepel)
...
cold_pie <- ggplot(cold_pieplot, aes(y = Estimate, x = "", fill = Variable)) + 
  ...
  scale_fill_manual(values = c("#046C9A", "#D69C4E" , "#000000")) + 
  theme_void() + 
  labs(title = "Cold Incubation Treatment") + 
  geom_label_repel(data = cold_pieplot) +
  annotate("text", y = pos, label = TeX(full_lab))
  
  data = cold_pieplot,
                   aes(y = pos, label = TeX(full_lab)),
                   size = 3.5, nudge_x = 1, show.legend = FALSE, parse = TRUE)
...
hot_pie <- ggplot(hot_pieplot, aes(y = Estimate, x = "", fill = Variable)) + 
  ...
  coord_polar("y", start=0) + 
  scale_fill_manual(values = c("#046C9A", "#D69C4E" , "#000000")) + 
  theme_void() + labs(title = "Hot Incubation Treatment")
